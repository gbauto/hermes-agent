...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    launchd plist XML formatting is not semantic.  Compare a canonical plist
    representation instead of raw text so indentation-only differences do not
    make ``hermes gateway status --deep`` report a stale service definition.
    If plist parsing fails, fall back to the historical text normalizer plus
    PATH masking so malformed files are still compared conservatively.
    """
    import copy
    import plistlib
    import re
