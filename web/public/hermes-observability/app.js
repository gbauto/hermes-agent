...
const STATE = {
  ...
  view: "swimlane", mode: "form", pool: "smoke-client", tag: "", search: "", sort: "latest", hideAfter: "30m", showHidden: false,
...
const OBS_TIME_ZONE = "America/Los_Angeles";
...
function apiUrl(path, params = {}) {
  const apiPath = path.startsWith("/api/") ? path : `/api/gbauto/pi-observability${path}`;
  const u = new URL(apiPath, location.origin);
...
function fmtTs(ts) {
  try { return new Date(ts).toLocaleTimeString("en-US", { hour12: false, hour:"2-digit", minute:"2-digit", second:"2-digit", timeZone: OBS_TIME_ZONE }); }
