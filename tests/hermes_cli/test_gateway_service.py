...
class TestLaunchdPlistNormalization:
    def test_launchd_plist_comparison_ignores_formatting_only_drift(self, tmp_path, monkeypatch):
        """Formatting-only XML differences must not make `status --deep` stale.

        launchd's plist parser treats indentation and newlines between elements as
        insignificant. The stale-check should compare the plist semantics too, not
        raw whitespace, while still masking PATH's generated shell-dependent value.
        """
